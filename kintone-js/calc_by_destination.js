...
(function() {
  ...
  kintone.events.on('app.record.index.show', function(event) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      if( record.destination.value == "送迎先場所未決定" )   continue;
     
      flg = 0;
      for(j=0; j< output.length; j++){
        if(record.destination.value == output[j][0]){
      ...
      if( flg != 1 ){
        ...
        output[j][0] = record.destination.value;   //人名
    ...
    for (k = 0; k < output.length; k++) {
      ...
      */
      ...
      //cell1.innerHTML = record.destination.value;    //送迎者名
